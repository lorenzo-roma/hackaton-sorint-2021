...
export default class ToBeScheduledTrip extends  Trip implements ToBeScheduledTripInterface {
    ...
    static fromTripResult(tripResult: TripResult) {
        return new ToBeScheduledTrip(tripResult.id, tripResult.from, tripResult.to, tripResult.arrival, tripResult.initial_availability!, tripResult.end_availability!)
    }
}

export interface ToBeScheduledTripInterface {
    from: string;
    to: string;
    arrival: Date;
