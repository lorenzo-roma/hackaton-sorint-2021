        ...
        <Router>
            <div className="min-vh-100">
                        ...
                        return (
                            <>
                                ...
                                !authStore.user?.driver && (
                                    <HomePagePrivate
                                        initialFrom={state.initialFrom}
                                        initialTo={state.initialTo}
                                    />
                                )}
                                {authState === AuthState.LOGGED_IN &&
                                authStore.user?.driver && (
                ...
                <Route exact path="/driver">
                    {(authState == AuthState.LOGGED_OUT ||
                        !authStore.user?.driver) && <DriverPublicPage/>}
                ...
                <Route exact path="/driver/login">
                    {authState == AuthState.LOGGED_IN &&
                    !authStore.user?.driver && <Redirect to="/"/>}
                    {authState == AuthState.LOGGED_IN && authStore.user?.driver && (
                        <Redirect to="/driver/"/>
                    )}
                    {authState == AuthState.LOGGED_OUT && (
                ...
                <Route
                    exact
                    path="/driver/shift/:id"
                    render={({match}) => {
                        const idNumber = parseInt(match.params.id);
                        if (authState == AuthState.LOGGED_OUT || isNaN(idNumber))
