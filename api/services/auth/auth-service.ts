...
export default class AuthService implements AuthServiceInterface {
  ...
  async signUp(user: User): Promise<ServiceResponse<AuthResult, User>> {
    ...
    if (userWithSameName != null) return { status: AuthResult.ALREADY_SIGN_UP };
  ...
  async logIn(user: User): Promise<ServiceResponse<AuthResult>> {
    const userToAuth = await this.repository.findUserByUsername(user.username);
  ...
  async verifyToken(token: string): Promise<ServiceResponse<AuthResult, User>> {
    try {
